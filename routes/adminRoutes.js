...
router.post(
  ...
  async (req, res) => {
    ...
    if (!file || !employeeId) return res.status(400).json({ error: "file and employeeId required" });
    ...
    const result = await tx(async (client) => {
      ...
      const rows = parseExcelBufferToJson(file.buffer);
      if (!rows.length) return { fileId, count: 0 };
      ...
      for (const r of rows) {
        values.push(`($${i++}, $${i++}, $${i++})`);
        params.push(fileId, employeeId, JSON.stringify(r));
      }
      ...
      return { fileId, count: rows.length };
    });
